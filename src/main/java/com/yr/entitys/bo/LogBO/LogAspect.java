package com.yr.entitys.bo.LogBO;
...
public class LogAspect {
     ...
     * 管理员登录方法的切入点
     */
    @Pointcut("execution(* com.yr.*.service.impl.*.*(..))")
    public void loginCell(){}

    /**
     * 添加业务逻辑方法切入点
     */
    @Pointcut("execution(* com.yr.*.service.impl.*.add*(..))")
    public void insertCell() {}

    /**
     * 修改业务逻辑方法切入点
     */
    @Pointcut("execution(* com.yr.*.service.impl.*.update*(..))")
    public void updateCell() {}

    /**
     * 删除业务逻辑方法切入点
     */
    @Pointcut("execution(* com.yr.*.service.impl.*.delete*(..))")
    public void deleteCell() {}

    /**
     * 导入业务逻辑方法切入点
     */
    @Pointcut("execution(* com.yr.*.service.impl.*.import*(..))")
    public void importCell() {}

    /**
     * 导出业务逻辑方法切入点
     */
    @Pointcut("execution(* com.yr.*.service.impl.*.export*(..))")
    ...
    public void loginLog(JoinPoint joinPoint, Object object) throws Throwable {
        ...
        //产生日志的表名
        log.setTable("");
        //模块操作的类型0抛异常，1新增，2删除，3修改，4用户登录，5用户退出
        log.setType(5);
        //修改之前数据
        log.setFieldOldValue("");
        //修改之后数据
        log.setFieldNewValue("");
        //记录异常内容
        log.setContent("");
