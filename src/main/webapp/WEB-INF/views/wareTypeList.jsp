...
<script>
    ...
    layui.config({
        ...
        ptable=treeGrid.render({
            ...
            ,elem: '#'+tableId
            ,idField:'id'
            ,url:'<%=request.getContextPath() %>/ware_type/ware_typeTable/list'
            ,cellMinWidth: 100
            ,treeId:'id'//树形id字段名称
    ...
    function del(obj) {
        layer.confirm("你确定删除数据吗？如果存在下级节点则一并删除，此操作不能撤销！", {icon: 3, title:'提示'},
            function(index){//确定回调
                ...
                $.ajax({
                    type: "post",
                    url: "<%=request.getContextPath()%>/ware_type/ware_typeTable/"+obj.id,
                    data: {"_method": "delete"},
                    error: function () {
                        layer.msg("操作失败",{icon:2});
                        setTimeout(function(){
                            window.location.href='<%=request.getContextPath() %>/ware_type/ware_typeTable';
                        },2000);
                    },
    ...
    function upd(uObj) {
        layer.open({
            ...
            area : ['800px' , '520px'],
            content: '<%=request.getContextPath() %>/ware_type/ware_typeTable/'+uObj.id,
            end: function(){
                window.location.href='<%=request.getContextPath() %>/ware_type/ware_typeTable';
            }
        });
    ...
    function add(pObj) {
        ...
        layer.open({
            ...
            shadeClose: true, //点击遮罩关闭层
            area : ['800px' , '520px'],
            content: '<%=request.getContextPath() %>/ware_type/ware_typeTable/add',
            end: function(){
                window.location.href='<%=request.getContextPath() %>/ware_type/ware_typeTable';
            }
        });
