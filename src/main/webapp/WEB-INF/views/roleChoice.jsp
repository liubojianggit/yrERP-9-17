...
<head>
    ...
    <script type="text/javascript">
        ...
        window.onload=function () {
            $.ajax({
                type: 'post',
                url: '<%=request.getContextPath() %>/',//请求后台拿到角色
                dataType : 'json',
                success: function(data){
                    ...
                    for ( var i in data) {
                        tt++;
                        if(data[i].mark == true){
                            strTr+="<td>"+"<input type=\"checkbox\" checked lay-skin=\"switch\" lay-filter=\"encrypt\" title=\""+data[i].unicode+"\">"+"</td>";
                        }else {
                            strTr+="<td>"+"<input type=\"checkbox\" checked lay-skin=\"switch\" lay-filter=\"encrypt\" title=\""+data[i].unicode+"\">"+"</td>";
                        }
                        ...
                        if(tt%4==0){//一行7个列
                            ...
                            if((data.length - tt) <= (data.length)%4){//最后一行
