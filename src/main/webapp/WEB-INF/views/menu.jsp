...
<body class="layui-layout-body" style="overflow:auto; padding: 10px">
    <a class="layui-btn layui-btn-primary"  onclick="window.location.href='<%=request.getContextPath() %>/menu';">刷新</a>
    <a class="layui-btn layui-btn-primary"  onclick="add();">新增菜单</a>
...
<script>
    ...
    layui.config({
        ...
        ptable=treeGrid.render({
            ...
            ,elem: '#'+tableId
            ,idField:'id'
            ,url:'<%=request.getContextPath() %>/menu/menuTable/list'
            ,cellMinWidth: 100
            ,treeId:'id'//树形id字段名称
            ,treeUpId:'pid'//树形父id字段名称
            ,treeShowName:'title'//以树形式显示的字段
            ,cols: [[
                {field:'title', edit:'title',width:'15%', title: '菜单名称'},
                {field:'icon',width:'15%', title: '图标'},
                {field:'href',width:'20%', title: 'url'},
    ...
    function del(obj) {
        layer.confirm("你确定删除数据吗？如果存在下级节点则一并删除，此操作不能撤销！", {icon: 3, title:'提示'},
            function(index){//确定回调
                ...
                $.ajax({
                    type: "post",
                    url: "<%=request.getContextPath()%>/menu/menuTable/"+obj.id,
                    data: {"_method": "delete"},
                    error: function () {
                        layer.msg("操作失败",{icon:2});
                        setTimeout(function(){
                            window.location.href='<%=request.getContextPath() %>/menu/menuTable';
                        },2000);
                    },
    ...
    function upd(uObj) {
        layer.open({
            ...
            area : ['800px' , '520px'],
            content: '<%=request.getContextPath() %>/menu/menuTable/'+uObj.id,
            end: function(){
                window.location.href='<%=request.getContextPath() %>/menu/menuTable';
            }
        });
    ...
    function add(pObj) {
        ...
        layer.open({
            ...
            shadeClose: true, //点击遮罩关闭层
            area : ['800px' , '520px'],
            content: '<%=request.getContextPath() %>/menu/menuTable/add',
            end: function(){
                window.location.href='<%=request.getContextPath() %>/menu/menuTable';
            }
        });
