...
<form:form class="layui-form" style="width:80%;" id="form2" method="POST" modelAttribute="suppWareTypeBo">
    <form:errors path="*"></form:errors>
    <c:if test="${suppWareTypeBo.suppWareType.id != null }">
        <input type="hidden" name="_method" value="PUT"/>
        <input type="hidden" name="id" value="${suppWareTypeBo.suppWareType.getId() }">
    </c:if>
    <div class="layui-form-item layui-row layui-col-xs12">
        <label class="layui-form-label">商品类型名称</label>
        <div class="layui-input-block">
            <form:input path="suppWareType.name" class="layui-input"  lay-verify="required" placeholder="请输入商品类型名："/>
        </div>
    </div>
    <div class="layui-form-item layui-row layui-col-xs12">
        <label class="layui-form-label">商品类型编号</label>
        <div class="layui-input-block">
            <form:input path="suppWareType.code" class="layui-input"  lay-verify="required" placeholder="请输入商品类型编号："/>
        </div>
    </div>
    ...
    <div class="magb15 layui-col-md4 layui-form-item layui-col-xs12">
        ...
        <div class="layui-input-inline">
            <form:select path="suppWareType.supCode" id="pidSelect" items="${supWareType }" itemLabel="name" itemValue="id"></form:select>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        if (${suppWareTypeBo.suppWareType.id == null }){
            $("#pidSelect").val(0);
        }
    })
